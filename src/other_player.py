...
class OtherPlayer(Entity):
    ...
    def import_player_assets(self, character):
        image = pygame.image.load(
            f"./graphic/Characters/{character}/SpriteSheet.png"
        ).convert_alpha()
        self.animations = {
            "down": [
                pygame.transform.scale(
                    image.subsurface((0, i, 16, 16)), (TILESIZE, TILESIZE)
                )
                for i in range(0, 49, 16)
            ],
            "up": [
                pygame.transform.scale(
                    image.subsurface((16, i, 16, 16)), (TILESIZE, TILESIZE)
                )
                for i in range(0, 49, 16)
            ],
            "left": [
                pygame.transform.scale(
                    image.subsurface((32, i, 16, 16)), (TILESIZE, TILESIZE)
                )
                for i in range(0, 49, 16)
            ],
            "right": [
                pygame.transform.scale(
                    image.subsurface((48, i, 16, 16)), (TILESIZE, TILESIZE)
                )
                for i in range(0, 49, 16)
            ],
        }
